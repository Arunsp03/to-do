...
const {saveEditedTask}=ApiService;
...
// }
const toggleTaskForm = (task: string | null) => {
        
    setEditMode((prev) => (prev === task ? null : task));
  };
const submitEditedTask=async (id:number)=>{
    try{

    const response=await saveEditedTask(EditForm,id);
    props.fetchData();
    toggleTaskForm(null)
    // toggleEditForm();
